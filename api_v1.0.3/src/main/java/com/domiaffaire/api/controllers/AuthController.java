...
public class AuthController {
    ...
    public ResponseEntity<?> createCustomer(@RequestPart("image") MultipartFile file,
        ...
        try {
            ...
            if (createdUser != null) {
                ...
                return ResponseEntity.status(HttpStatus.CREATED).body("Check your mail to verify your account");
            } else {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Failed to create user.");
            }
        } catch (IOException e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("Failed to process image file.");
        } catch (WrongCodeComptableException e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Code comptable does not match.");
        }
    }
    ...
    public ResponseEntity<?> resetPasswordRequest(@RequestBody PasswordResetRequest passwordResetRequest,final HttpServletRequest request) throws MessagingException, UnsupportedEncodingException {
        ...
        if(user.isPresent()){
            ...
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("User does not exist with this email"+passwordResetRequest.getEmail());
        }
