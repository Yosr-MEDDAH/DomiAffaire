...
export class DomiciliationRequestsComponent implements OnInit {
  ...
  search() {
    if (this.searchFirstName == '') {
      ...
      this.domiciliationRequests = this.domiciliationRequests.filter(
        (res: { client: { firstName: string } }) => {
          return res.client.firstName
            .toLocaleLowerCase()
            .includes(this.searchFirstName.toLocaleLowerCase());
        }
      );
    }
  }
