...
export class ConsultationComponent implements OnInit {
      ...
      next: (data: any) => {
        this.consultations = data.map((item: any) => ({
          ...
          proposedDate: item.proposedDate
            ? this.parseProposedDate(item.proposedDate)
            : null,
          finalConsultationDate: item.finalConsultationDate
            ? this.parseSentDate(item.finalConsultationDate)
            : null,
        }));
      },
  ...
  confirmCancelRequest() {
    this.clientService.cancelConsultationRequest(this.requestId).subscribe({
      next: (data: any) => {
