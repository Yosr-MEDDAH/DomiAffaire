...
export class DomiciliationRequestComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private clientService: ClientService, private toastService:ToastService) {}
  ngOnInit(): void {
    this.initForm();
    ...
    if (this.domiciliationForm.get('legalForm')?.value === 'Natural person') {
      ...
      this.clientService.sendDomiciliationPP(formData).subscribe({
        next: (data: any) => {
          ...
          this.toastService.showToast('success',"Domiciliation Request Sent Successfully")
        },
  ...
  private setRequiredValidatorsForInProcess() {
    this.domiciliationForm.get('draftStatus')?.setValidators([Validators.required]);
    this.domiciliationForm.get('shareCapital')?.setValidators([Validators.required]);
    this.domiciliationForm.get('management')?.setValidators([Validators.required]);
    this.domiciliationForm.get('extractRNE')?.setValidators([Validators.required]);
  ...
  private setRequiredValidatorsForTransfer() {
    ...
    this.domiciliationForm.get('management')?.setValidators([Validators.required]);
    this.domiciliationForm.get('extractRNE')?.setValidators([Validators.required]);
    this.domiciliationForm.get('oldDraftStatus')?.setValidators([Validators.required]);
    this.domiciliationForm.get('oldLegalForm')?.setValidators([Validators.required]);
    this.domiciliationForm.get('oldShareCapital')?.setValidators([Validators.required]);
    this.domiciliationForm.get('oldManagement')?.setValidators([Validators.required]);
    this.domiciliationForm.get('pvChangeAddress')?.setValidators([Validators.required]);
    this.domiciliationForm.get('oldBusinessLicence')?.setValidators([Validators.required]);
    this.domiciliationForm.get('oldExistenceDeclaration')?.setValidators([Validators.required]);
  }
  
  private clearValidators() {
    const fields = ['draftStatus', 'shareCapital', 'management', 'extractRNE', 
                    'oldDraftStatus', 'oldLegalForm', 'oldShareCapital', 'oldManagement', 
                    'pvChangeAddress', 'oldBusinessLicence', 'oldExistenceDeclaration'];
  ...
  private clearValidatorsForTransferFields() {
    const fields = ['oldDraftStatus', 'oldLegalForm', 'oldShareCapital', 'oldManagement', 
                    'pvChangeAddress', 'oldBusinessLicence', 'oldExistenceDeclaration'];
